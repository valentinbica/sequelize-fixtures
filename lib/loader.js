...
Loader.prototype.loadFixture = function(fixture, models) {
  ...
  return this.prepFixtureData(fixture.data, Model).spread(function(data, many2many) {
    ...
    if (self.options.useOrOperatorWithTableNames && self.options.useOrOperatorWithTableNames.indexOf(Model.name) !== -1) {
      finalWhere = {[self.options.orOperator]: where};
    }
    var findOptions = {where: finalWhere};
    ...
    return Model.find(findOptions).then(function(instance) {
      if (instance) {
        self.skipped++;
        var rawInstance = instance.get({raw: true});
        var makeUpdate = false;
        for (var key in data) {
          if (rawInstance[key] !== data[key]) {
            makeUpdate = true;
            break;
          }
        }
